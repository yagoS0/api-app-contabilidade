...
Rodada de integração: recaptura conserva a última observação e sua data; versão alterada exige nova consulta, mantendo histórico. Parcelas declaradas CLIENTE são elegíveis à conferência oficial sem reabrir pagamento ou cobrar automaticamente. Retorno recusado para a guia não confirma sua parcela; o intervalo da tentativa paga permanece. “Já paguei” no WhatsApp encaminha à equipe e não lista guias pagas nem altera estado fiscal. Perguntas sobre histórico continuam consultas. Cenários PostgreSQL e de conversa entram no CI; simuladores bloqueiam rede externa.
...
Consulta externa não deve impedir contratação com evidência alternativa: consulta pública manual registra fonte/CNPJ/evidência e diagnóstico privado dispensado explicita limitação no PDF. Não marcar consulta inexistente como realizada. SITFIS reaproveita prova oficial recente da procuração do mesmo caso/procurador, preserva protocolo e só conclui com PDF armazenado. Janela WhatsApp considera a entrada efetiva mais recente do mesmo número e canal, mesmo com webhook fora de ordem. Preços ajustáveis antes do aceite; contrato mantém os valores aceitos. Testes sem Anthropic, envio real ou consulta fiscal paga; banco de testes separado. Contextos: `docs/chat-atendimento-20260921-revisao.md`, `docs/jornada-lead-passo-a-passo.md`.
