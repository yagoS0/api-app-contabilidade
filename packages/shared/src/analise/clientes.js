...
export function montarClientes({notas,de,ate,comparar='anterior',hoje,competenciasFechadas}) {
  ...
  for(const nota of notas){
    ...
    if(m>ate||(fechados&&!fechados.has(m)))continue;
  ...
  const clientes=[...grupos.values()].map(g=>{
    ...
    const recorrente=!recorrenciaDisponivel?null:meses(deslocarMes(ate,-3),ate).filter(m=>(g.porMes.get(m)||0)>0).length>=3;
    const referencia=!fechados&&ate>=hoje.slice(0,7)?deslocarMes(hoje.slice(0,7),-1):ate;
    ...
    const disparidade=janelaFechada(deslocarMes(referencia,-6),referencia)&&habitual>0&&Math.abs(ultimoValor/habitual-1)>=.3?{mes:referencia,atual:ultimoValor/100,mediana:habitual/100,variacao:(ultimoValor/habitual-1)*100}:null;
  ...
  if(!comparacaoDisponivel){for(const c of clientes){c.anterior=null;c.variacao=null;c.diferenca=null;c.situacao=c.atual>0?'Com faturamento no período':'Somente histórico';}for(const k of Object.keys(ponte))ponte[k]=null;}
  return {comparacaoDisponivel,recorrenciaDisponivel,competenciasFechadas,avisos:fechados?['Histórico restrito às competências com fechamento contábil. Lacunas não representam ausência de faturamento nem LTV completo.']:[],de,ate,anterior,hoje,inicioHistorico,parcial:!fechados&&ate>=hoje.slice(0,7),semIdentificacao,valorSemIdentificacao:valorSemIdentificacao/100,invalidas,clientes,ponte,
    ...
    resumo:{total,anterior:comparacaoDisponivel?base:null,top1:total>0?soma(ativos.slice(0,1).map(c=>c.atual))/total*100:null,top3:total>0?soma(ativos.slice(0,3).map(c=>c.atual))/total*100:null,top3Anterior:comparacaoDisponivel&&base>0?soma([...clientes].sort((a,b)=>b.anterior-a.anterior).slice(0,3).map(c=>c.anterior))/base*100:null,taxaRecorrencia:recorrenciaDisponivel&&ativos.length?ativos.filter(c=>c.recorrente).length/ativos.length*100:null,receitaRecorrentes:!recorrenciaDisponivel?null:soma(ativos.filter(c=>c.recorrente).map(c=>Math.round(c.atual*100)))/100,ativos:ativos.length,media:ativos.length?total/ativos.length:null,ticket:totalNotas?total/totalNotas:null,top5:total>0?soma(ativos.slice(0,5).map(c=>c.atual))/total*100:null,novos:ativos.filter(c=>c.primeira>=de).length,semFaturamento:!comparacaoDisponivel?null:clientes.filter(c=>c.anterior>0&&c.atual===0).length,recorrentes:!recorrenciaDisponivel?null:ativos.filter(c=>c.recorrente).length,maior:ativos[0]||null,menor:ativos.at(-1)||null},
