...
// Contrato ViaCEP: https://viacep.com.br/ (mesmo provedor do emissor do contador).
...
import { CAMPOS_ENDERECO_EXIGIDOS, codigoMunicipioVerificado } from "../../features/emitir/lib/consultaTomador";
...
export async function consultarCep(cep, { fetchImpl = null, municipios = null, timeoutMs = TIMEOUT_MS, log = null } = {}) {
  ...
  if (typeof buscar !== "function") return falha(MOTIVOS.SEM_FETCH, "A consulta de CEP está indisponível neste navegador.", codigo);
  ...
  try {
    resultado = await Promise.race([
      ...
      (async () => {
        ...
        const lista = municipios ?? await carregarMunicipiosIbge().catch(() => []);
        const municipio = codigoMunicipioVerificado({ codigo_municipio_ibge: ibge, municipio: municipioTexto, uf }, lista);
        const endereco = {
