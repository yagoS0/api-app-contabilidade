// ⚠ ORIGEM: `apps/web/src/features/notas/lib/valorDaNota.js` (portal do escritório). MESMA REGRA,
// apps separados e sem código compartilhado — mesmo arranjo de `consultaTomador.js` e
// `municipioIbge.js` aqui do lado. Mudou lá, muda aqui.
//
// ⚠ O QUE ESTE PORTAL TINHA ERA DIFERENTE, E A DECISÃO ANTERIOR ESTÁ REVERTIDA AQUI. O campo era
// `<input type="number" step="0.01">`, e o comentário de `EmitirNotaPage` dizia, com razão para a
// época: *"máscara de moeda em pt-BR precisa decidir se 1.234 é mil duzentos e trinta e quatro ou
// um vírgula dois — e essa decisão, errada, vira o VALOR DA NOTA"*. O dono pediu o campo com
// vírgula (18/08/2026), e a máscara resolve aquele receio pelo outro lado: ela não DECIDE nada,
// porque a grafia ambígua não pode ser digitada. O que restava do receio era a COLAGEM — e é
// exatamente ela que `lerValorColado` recusa em vez de adivinhar.
//
