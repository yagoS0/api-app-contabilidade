...
describe('painel sem avisos de guias vencidas', () => {
  test.each([true, false])('não mostra popup nem bloco de guias com ackPending=%s', async (ackPending) => {
    const registrar = jest.spyOn(api, 'registrarCienciaDeGuias').mockResolvedValue({ ok: true });
    const p = cheio();
    await abrir({ ...p, alertaDeGuias: { ...p.alertaDeGuias, ackPending } });
    expect(screen.queryByRole('alertdialog')).not.toBeInTheDocument();
    expect(screen.queryByRole('button', { name: /Ver todas as guias|Estou ciente/ })).not.toBeInTheDocument();
    expect(screen.queryByText(/guia[s]? para pagar|venceu em/i)).not.toBeInTheDocument();
    expect(screen.getByRole('button', { name: /Ver saídas de agosto/ })).toBeInTheDocument();
    expect(registrar).not.toHaveBeenCalled();
    expect(api.getFluxoCaixa).toHaveBeenCalledTimes(1);
  });

  test('alternar para DRE não reapresenta avisos nem consulta guias', async () => {
    const guias = jest.spyOn(api, 'getGuides').mockResolvedValue({ items: [] });
    jest.spyOn(api, 'getDre').mockResolvedValue({ competenciasDisponiveis: [], semLancamento: true });
    await abrir(cheio());
    await clicar('DRE');
    expect(screen.queryByRole('alertdialog')).not.toBeInTheDocument();
    expect(screen.queryByRole('button', { name: /Ver todas as guias|Estou ciente/ })).not.toBeInTheDocument();
