// LISTA OFICIAL DE MUNICÍPIOS DO IBGE — DADO, não regra. Gerado, não escrito à mão.
//
// FONTE: https://servicodados.ibge.gov.br/api/v1/localidades/municipios (API de Localidades do IBGE)
// EXTRAÍDO EM: 2026-08-14
...
//
// ⚠⚠ ESTA É A SEGUNDA CÓPIA DESTE ARQUIVO NO REPOSITÓRIO. A primeira é
// `apps/web/src/lib/municipios/municipiosIbge.data.js` (portal do escritório), e as duas são
// BYTE A BYTE a mesma tabela, extraída na mesma data. **Regenerou uma, regenere a outra** — o
// procedimento está abaixo e vale para as duas.
//
// POR QUE HÁ DUAS, E O QUE FALTA PARA HAVER UMA SÓ (decisão registrada em 18/08/2026):
// o lugar certo desta tabela é um pacote em `packages/` consumido pelos dois apps. Criar o pacote
// AGORA não resolveria nada: o segundo consumidor é `apps/web`, e apontá-lo para o pacote exige
// editar `apps/web` — o que esta entrega não podia fazer. O resultado seria as duas cópias de hoje
// **mais** um terceiro lugar. Então a duplicação é assumida, com o aviso acima no cabeçalho das
// duas. Para consolidar depois: criar `packages/municipios-ibge` com este arquivo, rodar
// `npm install` (o workspace novo precisa do symlink em `node_modules/@contabilidade`), apontar
// `apps/web/src/lib/municipios/municipioIbge.js` e
// `apps/portal-cliente-web/src/lib/municipios/municipioIbge.js` para ele e apagar as duas cópias.
//
// ⚠ POR QUE ESTA LISTA EXISTE, E O QUE ELA NÃO FAZ.
// O código IBGE do município é dado fiscal: errado, a nota sai com o ISS apontado para o lugar
// errado, e o erro é silencioso. Converter nome→código por conta própria erra em homônimo — há
// CINCO "Bom Jesus" e cinco "São Domingos" nesta lista. Ela alimenta uma ESCOLHA de quem preenche;
// nada aqui deriva, sugere ou pré-seleciona município nenhum.
//
// ⚠ EMBARCADA DE PROPÓSITO, nunca buscada em tempo de execução: uma chamada de rede no meio de uma
// emissão seria mais uma dependência externa para falhar.
//
// ⚠ ESTA LISTA VAI ENVELHECER. Município é criado, incorporado e desmembrado — raramente, mas
// acontece. PARA ATUALIZAR (e troque a data acima pela da nova extração, NAS DUAS CÓPIAS):
//   1. curl -s "https://servicodados.ibge.gov.br/api/v1/localidades/municipios" -o municipios.json
...
export const MUNICIPIOS_IBGE = [
  ...
  ["3105509","Barão do Monte Alto","MG"],
  ...
  ["3165206","São Tomé das Letras","MG"],
  ...
  ["5107800","Santo Antônio de Leverger","MT"],
  ...
  ["1506500","Santa Izabel do Pará","PA"],
...
export default MUNICIPIOS_IBGE;
