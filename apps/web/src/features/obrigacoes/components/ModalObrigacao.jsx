...
export function ModalObrigacao({ empresas, opcoes, inicial, onFechar, onSalvar, salvando, erro }) {
  ...
  const unica = Boolean(inicial?.ocorrenciaId);
  ...
  return <Modal titulo={titulo} aoFechar={onFechar} ocupado={salvando} tamanho="md">
    <form onSubmit={(e) => {
      ...
      onSalvar({ ...form, verificador: form.tipo === "TAREFA" || avulsa ? "" : form.verificador, incluirVencidoDoMes: podeMarcarVencido && form.incluirVencidoDoMes });
      ...
      {(avulsa || unica) ? <>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(190px, 1fr))", gap: 12 }}>
          ...
          <Campo label="Dias corridos de preparação"><input required type="number" min="0" max="365" value={form.diasPreparacao} onChange={(e) => set("diasPreparacao", e.target.value)} style={campo} /></Campo>
        ...
        <div style={{ padding: 12, background: "var(--bg-page)", border: "1px solid var(--border)", borderRadius: 6, color: "var(--text-muted)" }}>
          ...
          {previa.proximas[0] && <p>Primeira janela de preparação: {fmt(preparar(previa.proximas[0], form.diasPreparacao))} até {fmt(previa.proximas[0])}.</p>}
