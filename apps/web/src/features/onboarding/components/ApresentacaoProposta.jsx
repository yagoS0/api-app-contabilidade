...
export function ApresentacaoProposta({ proposta, detalhada = false }) {
  ...
  return <>
    ...
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12 }}>{proposta.opcoes?.map(o => <article key={o.chave} style={{ flex: '1 1 220px', padding: 14, border: '1px solid var(--border)', borderRadius: 8 }}><strong>{o.titulo}</strong><p>{o.recorrente ? `${reais(o.mensalCentavos)}/mês${o.unicoCentavos !== 0 ? ` + ${reais(o.unicoCentavos)} pelo serviço inicial` : ''}` : `${reais(o.unicoCentavos)} pelo serviço`}</p><p style={bloco}>{o.escopo}</p>{o.regularizacaoIncluida ? <p>Regularização já incluída neste valor, sem cobrança duplicada.</p> : proposta.regularizacaoCentavos != null && <p>Regularização inicial: {reais(proposta.regularizacaoCentavos)}, além dos honorários desta opção.</p>}</article>)}</div>
    {proposta.decisaoRegularizacao?.necessaria === false && <p>O diagnóstico não identificou regularização anterior necessária no escopo conferido.</p>}
    {mensal && proposta.decisaoRegularizacao?.necessaria && <p>O início da contabilidade mensal fica condicionado à conclusão da regularização prevista nesta proposta.</p>}
    {!proposta.decisaoRegularizacao && proposta.regularizacaoCentavos == null && <p>Regularização: orçamento separado, quando necessária.</p>}
    <p>Taxas públicas: {reais(proposta.taxasCentavos)}{proposta.taxasConfirmadas ? ' (confirmadas)' : ' (a conferir)'}</p><p style={bloco}>{proposta.condicoes}</p>
  </>;
}
