...
export function CamposDaEtapa({ onboarding, campos, onSalvar, ocupado }) {
  ...
  if (!aberto) return <Button variant="secondary" disabled={ocupado} onClick={() => setAberto(true)}>Conferir ou preencher dados deste passo</Button>;
...
export function DiagnosticoDoLead({ jornada, onboarding, onSalvar, ocupado, limitado = false }) {
  ...
  if (!aberto) return <Button disabled={ocupado} onClick={() => setAberto(true)}>Preparar diagnóstico e escopo</Button>;
...
export function MensagemDoPasso({ api, conversaId, preparar, rotulo, onEnviado, disabled, enviado = false }) {
  ...
  if (enviada) return <p role="status">Mensagem enviada. <Button variant="secondary" disabled={disabled || ocupado} onClick={() => setEnviada(false)}>Preparar novamente</Button></p>;
