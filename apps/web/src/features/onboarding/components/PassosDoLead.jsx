...
export function ProgressoDoLead({ jornada, selecionado, onSelecionar, ocupado }) {
  return <nav className="lead-steps" aria-label="Passo a passo do lead"><ol>{jornada.passos.map((p, i) => <li key={p.id} data-status={p.concluido ? "concluido" : p.id === jornada.atual ? "atual" : "pendente"}>
    <button type="button" disabled={ocupado || !p.acessivel} aria-current={p.id === selecionado ? "step" : undefined} onClick={() => onSelecionar(p.id)}>
      <span aria-hidden="true">{p.concluido ? "✓" : i + 1}</span><span>{p.titulo}<small>{p.concluido ? "Concluído" : p.anterior ? "Registro anterior não disponível" : p.id === jornada.atual ? "Etapa atual" : "Pendente"}</small></span>
    </button></li>)}</ol></nav>;
}
...
export function CamposDaEtapa({ onboarding, campos, onSalvar, ocupado }) {
  ...
  return <fieldset disabled={ocupado} className="lead-step-fields"><legend>Dados deste passo</legend>{descritores.map(d => <CampoOnboarding key={d.campo} idPrefix="jornada" descritor={d} dados={dados} valor={dados[d.campo] ?? ""} onChange={v => {
    ...
    <Button disabled={!Object.keys(rascunho).length} onClick={async () => {
      ...
      if (await onSalvar({ versao: versaoEdicao.current, operacoes })) { setRascunho({}); versaoEdicao.current = null; }
...
export function MensagemDoPasso({ api, conversaId, preparar, rotulo, onEnviado, disabled }) {
  ...
  return <div className="lead-step-message"><Button variant="secondary" disabled={disabled || ocupado || !conversaId || incerto} onClick={() => executar(async () => setPrevia(await preparar()))}>{rotulo}</Button>
    {!conversaId && <p>Abra o atendimento na conversa do WhatsApp para enviar esta mensagem.</p>}
    {previa && <><p className="lead-message-preview">{previa.texto}</p><Button disabled={disabled || ocupado || incerto} onClick={() => executar(async () => {
      ...
      setPrevia(null); await onEnviado?.();
    })}>Conferi: enviar mensagem</Button></>}
    {erro && <p role="alert">{erro}</p>}{incerto && <p>Confira o histórico: não foi possível confirmar o resultado do envio.</p>}
  </div>;
}

export function OrientacaoDoPasso({ api, recursos, chaves, onboarding, conversaId, onEnviado, disabled }) {
  ...
  return <MensagemDoPasso key={`${r.id}:${onboarding.cnpj || ""}`} api={api} conversaId={conversaId} disabled={disabled} onEnviado={onEnviado} rotulo={`Preparar orientação: ${r.titulo}`} preparar={async () => {
