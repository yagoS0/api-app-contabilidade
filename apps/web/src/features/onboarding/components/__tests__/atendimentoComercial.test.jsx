...
test("trocar contato descarta prévia e inicializa nome/CNPJ da nova conversa", async () => {
  ...
  selecionar("r2");
  expect(screen.getByLabelText("Nome do destinatário")).toHaveValue("Contato B"); expect(screen.getByLabelText("CNPJ")).toHaveValue("11222333000181"); expect(screen.queryByLabelText("Orientação")).not.toBeInTheDocument();
  expect(api.comercial).toHaveBeenLastCalledWith("/recursos/r2/previa", { variaveis: { nome: "Contato B", cnpj: "11222333000181", servico: "" } }); expect(api.enviarOrientacaoWhatsapp).not.toHaveBeenCalled();
});
...
test("erro de listagem não aparece como biblioteca vazia", async () => {
  ...
  fireEvent.click(screen.getByRole("button", { name: "Mensagens rápidas" })); expect(await screen.findByRole("alert")).toHaveTextContent("Acesso indisponível"); expect(screen.queryByText(/Cadastre e aprove/)).not.toBeInTheDocument();
});
