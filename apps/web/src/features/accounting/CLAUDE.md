...
Pedido mais recente substitui a separação antiga entre lançamento e “Só entra no fluxo”: essa seção e o contador “no fluxo” foram removidos. A fila permite selecionar linhas prontas, preservando contas editadas, e confirmar o lote em Modal com resultado individual. Nunca repetir automaticamente um envio com resposta incerta. Ver `docs/a-lancar-simplificado-20260908.md` na raiz para regras e validação. As sugestões da IA continuam editáveis e não lançam sozinhas.

## Legibilidade — 08/09/2026

Etapa 2: exclusão individual/em lote e confirmação ERP em lote do Plano de Contas usam `useConfirmacao` (Modal compartilhado, lista das contas). Cancelar/Esc/desmontar não executa. Falhas de lote mantêm apenas suas contas selecionadas e nomeiam os códigos para repetir. Baixa de parcelas pagas também confirma pelo mesmo Modal, mantendo os valores e consequências anteriores. Circular: controles anuais 36px e textos de juros/multa e procedência com piso de 13px. Complemento desta etapa: reabertura do mês, repetição de consulta SERPRO, exclusão de função, repetição da folha, alertas/reabertura CSV individual e baixa manual/composição declarada usam o mesmo confirmador, preservando payload e consequências. Testes verificam cancelamento sem API, Esc preservando composição preenchida, confirmação explícita e período CSV. O hook tem camada 2000 para cobrir overlays legados da lista de funções (1500). As três rotinas restantes de parcelamento também usam o hook: busca de pagamento mantém integralmente o aviso de consulta PAGA, valores contratados em lote repetem todas as prestações e a aprovação lista as baixas selecionadas antes de mudar RASCUNHO para CONFIRMADO. Cancelar preserva seleção e valores digitados. As duas suítes afetadas passam 21 testes (parcelasDoAcordoBusca e confirmacoesLote); não restam chamadas executáveis de window.confirm nesta feature contábil. Isto padroniza confirmações; não implica conversão integral dos overlays legados. Não aninhar dois Modal compartilhados sem antes tratar sua pilha de Escape/foco.
