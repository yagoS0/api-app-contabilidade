...
export function ClientesRelatorio({api,empresaId,empresaNome,de,ate,comparar,embutido=false,limitarGraficoAoPeriodo=false}){
 ...
 const r=base&&limitarGraficoAoPeriodo?{...base,serie:base.serie.filter(m=>m.mes>=de&&m.mes<=ate),clientes:base.clientes.map(c=>({...c,serie:c.serie.filter(m=>m.mes>=de&&m.mes<=ate)}))}:base;
