...
export function ClientesRelatorio({api,empresaId,empresaNome,de,ate,comparar,embutido=false}){
 ...
 const chave=`${empresaId}|${de}|${ate}|${comparar}`,r=resposta?.chave===chave?resposta.dados:null;
 ...
 return <section className="clientes-report" aria-label="Relatório de clientes">
   ...
   {(r.parcial||r.semIdentificacao>0||r.invalidas>0)&&<div className="bi-aviso">{r.parcial&&<p>Período em andamento: as variações não representam fechamento mensal.</p>}{r.semIdentificacao>0&&<p>{r.semIdentificacao} nota(s), total de {brl(r.valorSemIdentificacao)}, sem CPF/CNPJ utilizável no período: fora dos indicadores por cliente.</p>}{r.invalidas>0&&<p>{r.invalidas} registro(s) inválido(s) fora da análise. Confira a origem antes de concluir sobre o crescimento.</p>}</div>}
   <div className="clientes-kpis bi-card"><div><span>Clientes com faturamento</span><strong>{s.ativos}</strong><small>{s.recorrentes==null?'Recorrência indisponível':`${s.recorrentes} com recorrência no histórico`}</small></div><div><span>Faturamento médio por cliente</span><strong>{brl(s.media)}</strong><small>{brl(s.total)} identificados no período</small></div><div><span>Concentração nos 5 maiores</span><strong>{parte(s.top5)}</strong><small>Participação no faturamento identificado</small></div><div><span>Novos no histórico</span><strong>{s.novos??'Sem base'}</strong><small>Primeira emissão observada no período</small></div><div><span>Sem faturamento no período</span><strong>{s.semFaturamento??'Sem base'}</strong><small>Tinham faturamento no período comparado</small></div></div>
   ...
   <div className="clientes-grade"><section className="bi-card"><h3>Distribuição do faturamento</h3><p>Participação dos maiores clientes na carteira identificada.</p>{top.length?<><div className="clientes-faixa" role="img" aria-label={`Cinco maiores: ${parte(s.top5)} do faturamento`}>{[...top.map(c=>({nome:c.nome,valor:c.atual})),{nome:'Demais clientes',valor:restante}].filter(l=>l.valor>0).map((l,i)=><span key={l.nome} title={`${l.nome}: ${brl(l.valor)}`} style={{width:`${l.valor/s.total*100}%`,background:['#bda2f3','#8f7aca','#647cb2','#799da2','#a0b4a6','#555568'][i]}}/>)}</div><ol className="clientes-ranking-curto">{top.map(c=><li key={c.documento}><button onClick={()=>setSelecionado(c.documento)}>{c.nome}</button><strong>{parte(c.participacao)}</strong></li>)}</ol><p>Demais clientes: {brl(restante)}</p></>:<p>Sem faturamento identificado para calcular concentração.</p>}</section><section className="bi-card"><Evolucao serie={r.serie} titulo="Faturamento identificado · 12 meses"/></section></div>
