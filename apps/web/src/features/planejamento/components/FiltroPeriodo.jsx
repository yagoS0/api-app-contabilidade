...
export function FiltroPeriodo({de,ate,comparar,onAplicar}) {
  ...
  const valido=/^\d{4}-(0[1-9]|1[0-2])$/.test(rascunho.de)&&/^\d{4}-(0[1-9]|1[0-2])$/.test(rascunho.ate)&&rascunho.de<=rascunho.ate&&n(rascunho.ate)-n(rascunho.de)<24;
  ...
  return <div ref={raiz} className="bi-periodo">
    <button ref={botao} type="button" className="bi-periodo-botao" aria-expanded={aberto} aria-controls={id} aria-haspopup="dialog" onClick={()=>{setRascunho({de,ate,comparar});setAberto(v=>!v);}}>
      ...
      <span>{de===ate?mes(de):`${mes(de)} — ${mes(ate)}`}</span><span aria-hidden="true">⌄</span><span className="bi-sr-only">Filtrar período</span>
    </button>
    {aberto&&<form id={id} className="bi-periodo-painel" role="dialog" aria-label="Filtrar período" onSubmit={e=>{e.preventDefault();if(valido){onAplicar(rascunho);fechar();}}}>
      <div className="bi-painel-titulo"><h2>Período</h2><button type="button" className="bi-icon-button" aria-label="Fechar filtros" onClick={fechar}>×</button></div>
      ...
      {!valido&&<p role="alert">Escolha um intervalo de até 24 meses.</p>}
