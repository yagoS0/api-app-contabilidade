...
export function FiltroPeriodo({de,ate,comparar,onAplicar,competenciasFechadas,limites=null,permitirLacunas=false}) {
  ...
  const selecionados=(competenciasFechadas||[]).filter(m=>m>=rascunho.de&&m<=rascunho.ate).sort();
  const valido=/^\d{4}-(0[1-9]|1[0-2])$/.test(rascunho.de)&&/^\d{4}-(0[1-9]|1[0-2])$/.test(rascunho.ate)&&rascunho.de<=rascunho.ate&&n(rascunho.ate)-n(rascunho.de)<24&&(permitirLacunas?selecionados.length>0:pendentes.length===0)&&(!limites||(rascunho.de>=limites.de&&rascunho.ate<=limites.ate));
  ...
  return <div ref={raiz} className="bi-periodo">
    ...
    {aberto&&<form id={id} className="bi-periodo-painel" role="dialog" aria-label="Filtrar período" onSubmit={e=>{e.preventDefault();if(valido){onAplicar(permitirLacunas ? {...rascunho,de:selecionados[0],ate:selecionados.at(-1)} : rascunho);fechar();}}}>
      ...
      {!valido&&<p role="alert">{permitirLacunas&&!selecionados.length?'Não há meses com fechamento contábil no período selecionado.':limites?'Escolha um intervalo dentro dos últimos 12 meses concluídos.':pendentes.length?`Sem fechamento contábil: ${pendentes.map(mes).join(', ')}. Confira em Lançamentos.`:'Escolha um intervalo de até 24 meses.'}</p>}
