...
export function AnaliseEmpresa({ api, empresaId, empresaNome, empresaCnpj }) {
  ...
  useEffect(() => { let vivo=true; setFechamentos(null); setErroFechamentos(''); setResposta(null); setDe(''); setAte('');
    ...
    Promise.resolve().then(()=>api.getFechamentosRelatorio(empresaId)).then(r=>{
      ...
      setDe(ultimo); setAte(ultimo); setFechamentos({empresaId,competenciasFechadas:meses});
  ...
  useEffect(() => { let vivo=true; setErro(''); setConta(null);
    if (!competenciasFechadas || !de || !ate) return;
  ...
  return <section className="analise-empresa bi-relatorios">
    <header className="bi-cabecalho"><div><h1>{secao==='projecao'?'Projeção financeira':'Relatórios da empresa'}</h1><span className="bi-contexto"> {empresaNome}</span></div><div className="bi-acoes">{secao==='projecao'?<button onClick={()=>setSecao('geral')}>Voltar aos relatórios</button>:<><button onClick={()=>setSecao('projecao')}>Projeção financeira ↗</button><FiltroPeriodo competenciasFechadas={competenciasFechadas||[]} de={de} ate={ate} comparar={comparar} onAplicar={v=>{setDe(v.de);setAte(v.ate);setComparar(v.comparar);}}/>{dados&&<ImprimirRelatorio key={chave} api={api} empresaId={empresaId} empresaNome={empresaNome} empresaCnpj={empresaCnpj} dados={dados} de={de} ate={ate} comparar={comparar}/>}</>}</div></header>
    {secao!=='projecao'&&<nav className="bi-tabs" aria-label="Seções da análise">{secoes.map(([k,n])=><button key={k} aria-pressed={secao===k||(k==='resultado'&&secao==='despesas')} onClick={()=>{setSecao(k);setConta(null);}}>{n}</button>)}</nav>}
    {secao==='projecao'?<><p className="bi-aviso">Área operacional · Usa dados atuais e previsões, inclusive competências abertas. Não integra os relatórios contábeis fechados. Acumulado projetado não representa saldo bancário.</p><FluxoLeitura key={empresaId} api={api} companyId={empresaId} titulo="Fluxo atual e compromissos"/></>:erroFechamentos || erro ? <div className="bi-card bi-estado" role="alert"><h2>Relatório indisponível</h2><p>{erroFechamentos || erro}</p><div className="bi-atalhos"><button onClick={()=>{setResposta(null);setFechamentos(null);setDe('');setAte('');setTentativa(n=>n+1);}}>Tentar novamente</button><a href={`/companies/${empresaId}/lancamentos`}>Conferir fechamentos nos lançamentos</a></div></div> : !competenciasFechadas ? <p role="status">Consultando fechamentos contábeis…</p> : !competenciasFechadas.length ? <section className="bi-card bi-estado"><h2>Nenhum mês fechado na contabilidade</h2><p>Feche uma competência nos lançamentos para disponibilizar os relatórios. O encerramento do mês no calendário não substitui o fechamento contábil.</p><a href={`/companies/${empresaId}/lancamentos`}>Conferir fechamentos nos lançamentos</a></section> : !dados ? <p role="status">Carregando análise…</p> : <>
      ...
      {anterior.indisponivel && <p className="bi-aviso">Comparação indisponível: feche a contabilidade de {anterior.mesesSemFechamento?.map(mes).join(', ')}. O período selecionado permanece disponível, sem indicadores de crescimento.</p>}
      <PendenciasRelatorio dados={dados} empresaId={empresaId}/>
      {secao==='clientes' && <ClientesRelatorio embutido key={empresaId} api={api} empresaId={empresaId} empresaNome={empresaNome} de={de} ate={ate} comparar={comparar}/>}
