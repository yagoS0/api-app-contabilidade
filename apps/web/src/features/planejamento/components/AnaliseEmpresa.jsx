import { cardsContabeis } from '../../../../../../packages/shared/src/analise/portalCliente.js';
...
const secoes = [['geral','Visão geral'],['clientes','Clientes'],['resultado','Resultado'],['impostos','Tributário']];
...
export function AnaliseEmpresa({ api, empresaId, empresaNome, empresaCnpj, portalCliente = false }) {
  ...
  useEffect(() => { let vivo=true; setFechamentos(null); setErroFechamentos(''); setResposta(null); setDe(''); setAte('');
    ...
    Promise.resolve().then(()=>api.getFechamentosRelatorio(empresaId)).then(r=>{
      ...
      setDe(portalCliente ? (meses[0] || r.de) : ultimo); setAte(portalCliente ? (meses.at(-1) || r.ate) : ultimo); setFechamentos({...r,empresaId,competenciasFechadas:meses});
    }).catch(e=>{if(vivo)setErroFechamentos(e.message||'Não foi possível consultar os fechamentos contábeis.');});
    return()=>{vivo=false;};
  },[api,empresaId,tentativa]);
  ...
  useEffect(() => { let vivo=true; setErro(''); setConta(null);
    if (!competenciasFechadas || !de || !ate || (portalCliente && !fechamentos?.liberado)) return;
    if (de>ate) { setErro('Informe um intervalo válido.'); return; }
    Promise.resolve().then(()=>api.getAnalisePlanejamento(empresaId,{de,ate,comparar})).then(r=>{if(!r || r.ok===false) throw new Error(r?.message||'Resposta inválida.'); if(vivo)setResposta({chave,dados:r});}).catch(e=>{if(vivo){setResposta(null);setErro(e.message||'Não foi possível carregar a análise.');}});
    return ()=>{vivo=false;};
  },[api,empresaId,de,ate,comparar,chave,tentativa,competenciasFechadas]);
  const atual = dados?.atual, anterior = dados?.anterior;
  const serie = (dados?.serie || []).filter(m => !portalCliente || (m.competencia >= de && m.competencia <= ate && competenciasFechadas?.includes(m.competencia))).slice(-12);
  ...
  const cards = cardsContabeis(atual?.dre);
  if (portalCliente && fechamentos && !fechamentos.liberado) return <section className="analise-empresa"><div className="bi-card bi-estado"><h2>Relatórios em preparação</h2><p>Disponíveis após o fechamento contábil dos três meses anteriores ao atual.</p><p>Aguardando: {fechamentos.pendentes?.map(mes).join(', ')}.</p><button onClick={()=>setTentativa(n=>n+1)}>Atualizar</button></div></section>;
  return <section className="analise-empresa bi-relatorios">
    <header className="bi-cabecalho"><div><h1>{secao==='projecao'?'Projeção financeira':'Relatórios da empresa'}</h1><span className="bi-contexto"> {empresaNome}</span></div><div className="bi-acoes">{secao==='projecao'?<button onClick={()=>setSecao('geral')}>Voltar aos relatórios</button>:<>{!portalCliente&&<button onClick={()=>setSecao('projecao')}>Projeção financeira ↗</button>}<FiltroPeriodo limites={portalCliente ? fechamentos : null} permitirLacunas={portalCliente} competenciasFechadas={competenciasFechadas||[]} de={de} ate={ate} comparar={comparar} onAplicar={v=>{setDe(v.de);setAte(v.ate);setComparar(v.comparar);}}/>{dados&&!portalCliente&&<ImprimirRelatorio key={chave} api={api} empresaId={empresaId} empresaNome={empresaNome} empresaCnpj={empresaCnpj} dados={dados} de={de} ate={ate} comparar={comparar}/>}</>}</div></header>
    {portalCliente&&dados&&<div className="bi-cards bi-kpis portal-resumo">{[['entradas','Entradas'],['saidas','Saídas'],['folha','Folha'],['impostos','Impostos'],['resultado','Resultado']].map(([k,n])=><div className="bi-card" key={k}><span>{n}</span><strong>{moeda(cards[k])}</strong><small>{mes(de)} a {mes(ate)}</small><small>{k==='impostos'?'Tributos e deduções da receita':k==='saidas'?'Custos e despesas, sem folha e tributos':k==='entradas'?'Receitas contabilizadas':k==='resultado'?'Entradas − saídas − folha − impostos':'Despesas com pessoal'}</small></div>)}</div>}{secao!=='projecao'&&<nav className="bi-tabs" aria-label="Seções da análise">{secoes.map(([k,n])=><button key={k} aria-pressed={secao===k||(k==='resultado'&&secao==='despesas')} onClick={()=>{setSecao(k);setConta(null);}}>{n}</button>)}</nav>}
    {secao==='projecao'?<><p className="bi-aviso">Área operacional · Usa dados atuais e previsões, inclusive competências abertas. Não integra os relatórios contábeis fechados. Acumulado projetado não representa saldo bancário.</p><FluxoLeitura key={empresaId} api={api} companyId={empresaId} titulo="Fluxo atual e compromissos"/></>:erroFechamentos || erro ? <div className="bi-card bi-estado" role="alert"><h2>Relatório indisponível</h2><p>{erroFechamentos || erro}</p><div className="bi-atalhos"><button onClick={()=>{setResposta(null);setFechamentos(null);setDe('');setAte('');setTentativa(n=>n+1);}}>Tentar novamente</button>{!portalCliente&&<a href={`/companies/${empresaId}/lancamentos`}>Conferir fechamentos nos lançamentos</a>}</div></div> : !competenciasFechadas ? <p role="status">Consultando fechamentos contábeis…</p> : !competenciasFechadas.length ? <section className="bi-card bi-estado"><h2>Nenhum mês fechado na contabilidade</h2><p>Feche uma competência nos lançamentos para disponibilizar os relatórios. O encerramento do mês no calendário não substitui o fechamento contábil.</p>{!portalCliente&&<a href={`/companies/${empresaId}/lancamentos`}>Conferir fechamentos nos lançamentos</a>}</section> : !dados ? <p role="status">Carregando análise…</p> : <>
      {dados.demonstracao && <p className="bi-aviso">Ambiente de demonstração · Dados fictícios para testes.</p>}
      {!portalCliente && <p className="bi-contexto">Contabilidade fechada · {mes(de)} a {mes(ate)}{(!portalCliente || !anterior.indisponivel) && <> · Comparação: {mes(anterior.de)} a {mes(anterior.ate)}</>}</p>}
      {!portalCliente && anterior.indisponivel && <p className="bi-aviso">Comparação indisponível: sem fechamento contábil em {anterior.mesesSemFechamento?.map(mes).join(', ')}. O período selecionado permanece disponível, sem indicadores de crescimento.</p>}
      {!portalCliente&&<PendenciasRelatorio dados={dados} empresaId={empresaId}/>}
      {secao==='clientes' && <ClientesRelatorio limitarGraficoAoPeriodo={portalCliente} competenciasFechadas={portalCliente ? competenciasFechadas : null} embutido key={empresaId} api={api} empresaId={empresaId} empresaNome={empresaNome} de={de} ate={ate} comparar={comparar}/>}
      {!portalCliente && (atual.parcial||(!anterior.indisponivel&&anterior.parcial)) && <p className="bi-aviso">Dados parciais · Confira a cobertura antes de interpretar o crescimento.</p>}
      {secao==='geral' && <>
        {!portalCliente&&<div className="bi-cards bi-kpis">{Object.entries(nomes).map(([k,n])=><button className="bi-card" key={k} onClick={()=>setSecao(destinos[k])}><span>{n}</span><strong>{ePercentual(k)?percentual(atual.indicadores[k]):moeda(atual.indicadores[k])}</strong><small>Anterior: {ePercentual(k)?percentual(anterior.indicadores[k]):moeda(anterior.indicadores[k])}</small><small><Variacao valor={dados.variacoes[k]} pontos={ePercentual(k)}/></small></button>)}</div>}
        <div className="bi-visao-grade"><section className="bi-card bi-evolucao-painel"><h2>{portalCliente?'Evolução mensal':'Evolução mensal · últimos 12 meses'}</h2><GraficoEvolucao serie={serie}/><details><summary>Ver valores mensais</summary><p>Faturamento documental e resultado contábil têm fontes diferentes. Ausências não são zero.</p><div className="bi-scroll"><table><thead><tr><th>Mês</th><th>Faturamento</th><th>Resultado da DRE</th><th>Margem líquida</th><th>Despesas</th></tr></thead><tbody>{serie.map(m=><tr key={m.competencia}><td>{mes(m.competencia)}</td><td>{moeda(m.indicadores.faturamento)}</td><td>{moeda(m.indicadores.resultado)}</td><td>{percentual(m.indicadores.margemLiquida)}</td><td>{moeda(m.indicadores.despesas)}</td></tr>)}</tbody></table></div></details></section>
        <section className="bi-card bi-referencias"><h2>Referências do período</h2><div className="bi-estatisticas">{[['Média mensal',estatistica.media],['Menor mês observado',estatistica.minimo],['Maior mês observado',estatistica.maximo],['Oscilação (desvio mensal)',estatistica.desvio]].map(([n,v])=><div key={n}><h3>{n}</h3><strong>{moeda(v)}</strong></div>)}</div><p>{estatistica.quantidade} mês(es) com registros; {estatistica.ausentes} sem base. Meses ausentes não entram como zero.</p></section></div>
        <section className="bi-card bi-mudancas"><h2>O que mudou?</h2>{[['Faturamento documental','faturamento'],['Resultado da DRE','resultado'],['Despesas','despesas']].map(([n,k])=>dados.variacoes[k]?.absoluta!=null&&<p key={k}>{n}: <Variacao valor={dados.variacoes[k]} pontos={ePercentual(k)}/></p>)}{dados.insights.length?dados.insights.map((i,n)=><p key={n}>{i.texto} <button onClick={()=>setSecao(i.secao)}>Ver composição</button></p>):<p>{atual.parcial||anterior.parcial?'Conclua a conferência dos períodos para habilitar alertas de evolução.':'Nenhuma regra de alerta acionada nos períodos comparados.'}</p>}</section>
      </>}
      ...
      {(secao==='resultado'||secao==='despesas') && <section className="bi-card"><h2>{secao==='resultado'?'DRE gerencial comparativa':'Despesas por categoria'}</h2><p className="bi-contexto">Competência: {mes(de)} a {mes(ate)}</p><p>Por competência e plano de contas. DRE gerencial, não é peça fiscal. Custos e IRPJ/CSLL ficam em linhas próprias da DRE.</p>
        ...
        <div className="bi-scroll"><table><thead><tr><th>Categoria / conta</th><th>Atual<br/><small>{mes(de)} a {mes(ate)}</small></th><th>Anterior<br/><small>{mes(anterior.de)} a {mes(anterior.ate)}</small></th><th>Variação</th>{secao==='despesas'&&<th>% da receita líquida</th>}</tr></thead><tbody>{atual.dre.linhas.filter(l=>secao==='resultado'||despesas.includes(l.chave)).map(l=>{const a=atual.dre.semLancamento?null:l.valor;const b=anterior.dre.semLancamento?null:anterior.dre.linhas.find(x=>x.chave===l.chave)?.valor; return <tr key={l.chave} className={l.chave==='receitaLiquida'||l.chave.startsWith('resultado') ? "bi-linha-total" : undefined}><td>{l.contas?.length?<details><summary>{l.rotulo}</summary>{l.contas.map(c=><p key={c.codigo}>{portalCliente?<span>{c.nome||c.codigo} · {c.reduzido}</span>:<button onClick={()=>setConta(c)}>{c.nome||c.codigo} · {c.reduzido}</button>} {moeda(c.valor)}</p>)}</details>:l.rotulo}</td><td>{moeda(a)}</td><td>{moeda(b)}</td><td>{a==null||b==null?'Sem base':moeda(a-b)}</td>{secao==='despesas'&&<td>{atual.dre.linhas.find(x=>x.chave==='receitaLiquida')?.valor>0?percentual(-a/atual.dre.linhas.find(x=>x.chave==='receitaLiquida').valor*100):'Sem base'}</td>}</tr>;})}</tbody></table></div>
      ...
      {!portalCliente&&secao==='resultado'&&<BaseGerencial key={empresaId} api={api} empresaId={empresaId} dre={atual.dre}/>}
      {!portalCliente&&secao==='resultado'&&<BaseSocios key={empresaId+de+ate} api={api} empresaId={empresaId} de={de} ate={ate} resultado={atual.indicadores.resultado}/>}
