...
export function FioDaConversa({ visivel = true, fio, hook, slotVincular = null, temMais = null, slotAcoes = null, hrefDaEmpresa = null, onVoltar = null, onDetalhes = null, detalhesAbertos = false, onCanalSelecionado = null, pedidoCanal = null, atualizacaoComercialExterna = null, usuarioId = null, onVirarAnotacao = null }) {
  ...
  const [empresaDaAcao, setEmpresaDaAcao] = useState("");
  ...
  useEffect(() => {
    const el = historicoRef.current;
    if (!el || !visivel || sobreposta || detalhesAbertos || buscando || escolherEmpresa || confirmarExclusao || fonteDaNota || typeof IntersectionObserver === "undefined") return;
    const visiveis = new Set();
    let ativo = true;
    const reconhecer = () => {
      if (!ativo || document.visibilityState === "hidden" || document.querySelector('[aria-modal="true"], .wa-quick-drawer:not([hidden]), .wa-lead-panel:not([hidden]), .wa-compose-sheet:not([hidden])')) return;
    ...
    return () => { ativo = false; observer.disconnect(); document.removeEventListener("visibilitychange", reconhecer); };
  }, [conversa.id, fio.mensagens, hook.marcarLida, detalhesAbertos, visivel, sobreposta, buscando, escolherEmpresa, confirmarExclusao, fonteDaNota]);

  useEffect(() => {
    const verificar = () => setSobreposta(Boolean(document.querySelector('[aria-modal="true"], .wa-quick-drawer:not([hidden]), .wa-lead-panel:not([hidden]), .wa-compose-sheet:not([hidden])')));
    const observer = new MutationObserver(verificar); verificar();
    observer.observe(document.body, { childList: true, subtree: true, attributes: true, attributeFilter: ["hidden", "aria-modal"] });
    return () => observer.disconnect();
  }, []);
  const empresasDaAcao = conversa.empresas?.length ? conversa.empresas : conversa.portalClientId ? [{ id: conversa.portalClientId, ...conversa.empresa }] : [];
  const acoesCompartilhadas = canal => <div>
    {empresasDaAcao.length > 1 && <label>Empresa desta operação<select value={empresaDaAcao} onChange={e => setEmpresaDaAcao(e.target.value)}><option value="">Escolha a empresa</option>{empresasDaAcao.map(e => <option key={e.id} value={e.id}>{e.razao}</option>)}</select></label>}
    {(empresaDaAcao || empresasDaAcao.length === 1) && <AcoesRapidas key={`${canal.id}:${empresaDaAcao}`} conversa={canal} mensagens={fio.mensagens} janela={canal.janela} api={hook.api} companyId={empresaDaAcao || empresasDaAcao[0]?.id} onEnviado={() => hook.atualizarConversa(conversa.id)} />}
  </div>;
  return (
    <AtualizacaoAtendimento.Provider value={atualizacaoComercialExterna || atualizacaoComercial}><div data-testid="fio" className="wa-thread" ref={fioRef} tabIndex={-1} aria-label={`Conversa com ${identidade.pessoa}`}>
      ...
      <div className="wa-messages" ref={historicoRef} onScroll={acompanharLeitura} aria-label="Histórico de mensagens" tabIndex={0}>
        {hook.contextoHistorico && <p role="status" className="wa-notice">Você está vendo uma mensagem encontrada na busca. <Button variant="secondary" size="sm" onClick={hook.voltarRecentes}>Voltar às mensagens recentes</Button></p>}
      ...
      <span role="status" className="wa-visually-hidden">{novas ? "Há novas mensagens nesta conversa." : ""}</span>
