...
export function useConversasWhatsapp({ api, feedback } = {}) {
  ...
  const carregar = useCallback(async (f = filtro) => {
    ...
    setCarregando(true);
    try {
      const r = await api.listarConversasWhatsapp(f);
  ...
  return { filtro, setFiltro, conversas, consumoIa, carregando, erro, aberta, carregandoFio, ocupado, carregar, abrir, assumir, devolver, responder, vincular, fechar: () => setAberta(null) };
}
