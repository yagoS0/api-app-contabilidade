  ...
  return <section className="nfse-profile-editor" aria-label="Editar perfis de emissão">
    ...
    {!form ? <>
      ...
      <fieldset disabled={salvando} style={{ border: 0, padding: 0 }}>
        ...
        {GRUPOS.map(([titulo, ids], index) => <details key={titulo} className="nfse-section" open={index === 0 || undefined}><summary><span className="nfse-step">{index + 1}</span>{" "}{titulo}</summary><div className="form-grid two-col">
          {ids.map((id) => {
            ...
            return <div key={id} className={id === "codigoServicoNacional" || id === "codigoNbs" ? "full" : undefined}>
              ...
              {id === "codigoServicoNacional" && dados.sugestoes?.porServico?.length ?
                <select id={`perfil-${id}`} required value={form[id] ?? ""} onChange={(e) => mudar(id, e.target.value)}>
                  {form[id] && !dados.sugestoes.porServico.some(s => s.codigo === form[id]) && <option value={form[id]}>{form[id]} — Valor salvo; confira os serviços habilitados</option>}
        ...
        <div className="nfse-profile-actions"><Button variant="primary" type="submit" disabled={salvando}>{salvando ? "Salvando…" : "Salvar perfil"}</Button>{" "}
        <Button type="button" variant="secondary" onClick={() => setForm(null)}>Cancelar edição</Button></div>
      </fieldset>
    </form>}
  </section>;
}
