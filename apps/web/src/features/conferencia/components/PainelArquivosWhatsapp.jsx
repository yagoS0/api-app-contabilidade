...
export function PainelArquivosWhatsapp({ api, companyId, contas = [], podeEscrever = false, aoImportar }) {
  ...
  return <section data-testid="arquivos-whatsapp" style={{ border: "1px solid var(--border)", borderRadius: 8, padding: 12 }}>
    ...
    <p style={{ color: "var(--text-muted)", fontSize: "0.8rem" }}>Abra e confira o original antes de importar. Arquivos comuns ficam disponíveis por 90 dias. Comprovantes vinculados às guias e lançamentos importados são preservados.</p>
    ...
    {arquivos.map(a => <div key={a.id} style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap", borderTop: "1px solid var(--border)", padding: "10px 0" }}>
      <div style={{ flex: 1, minWidth: 180 }}><strong>{a.nomeArquivo || "Arquivo sem nome"}</strong>{a.comprovanteGuiaId&&<p>{a.analiseComprovante?.guiaTipo || "Guia"} {a.analiseComprovante?.competencia} · Data informada: {a.dataPagamentoDeclarada?.slice(0,10).split("-").reverse().join("/")} · Aguardando conferência{a.analiseComprovante?.dataLida&&<> · Data lida: {a.analiseComprovante.dataLida.split("-").reverse().join("/")}</>}{a.analiseComprovante?.valorLido!=null&&<> · Valor lido: {Number(a.analiseComprovante.valorLido).toLocaleString("pt-BR",{style:"currency",currency:"BRL"})}</>}{a.analiseComprovante?.dataDivergente&&<strong> · Data divergente</strong>}</p>}<div style={{ color: "var(--text-muted)", fontSize: "0.76rem" }}>{a.importadoEm ? "Importação registrada" : ESTADO[a.estado] || a.estado || "Estado não informado"} · recebido em {data(a.recebidoEm)} · {a.comprovanteGuiaId ? "Comprovante vinculado à guia" : "original até " + data(a.expiraEm)}</div></div>
