...
// reescrita: ela já classifica a recusa em `{ ok:false, motivo, mensagem }`, já trata a ausência de
// `fetch` e já aceita `fetchImpl` para o teste não sair para a internet. O que mora aqui é o que a
// EMISSÃO decide em cima da resposta — e que o onboarding não decide, porque lá o destino é outro.
//
// ⚠ CPF NÃO SE CONSULTA. O campo do tomador é `cnpjCpf` e o tomador pode ser pessoa física. Com 11
// dígitos NADA acontece: sem chamada, sem "não encontrado", sem piscar. A BrasilAPI é base de CNPJ;
// perguntar por CPF é buscar o que não existe e devolver uma recusa que não significa nada.
//
// ⚠ A CONSULTA É AJUDA, NUNCA PORTÃO. Nenhuma função daqui devolve impedimento: falha de rede,
// CNPJ não encontrado ou API fora do ar não podem bloquear a emissão — o contador digita e segue.
// A recusa APARECE (o projeto tem o precedente ruim do erro engolido virando "o botão não faz
// nada"), mas o caminho continua aberto.
//
// ⚠ O QUE VEM DA API É SUGESTÃO; O CONTADOR MANDA. O nome que vai na nota é ato fiscal. Se ele
// digitou por cima, o digitado vence uma consulta nova — e a tela mostra os dois, como o
// planejamento tributário já faz (`da_empresa` × `digitado` × `ausente`).
...
export function aplicarNome({ nomeAtual = "", origemAtual = ORIGEM.AUSENTE, nome = "" } = {}) {
  ...
  if (!daReceita) {
    return { nome: atual, origem: origemAtual, aplicou: false, motivo: "a consulta não trouxe a razão social" };
  }
  if (origemAtual === ORIGEM.DIGITADO && atual.trim()) {
    return { nome: atual, origem: ORIGEM.DIGITADO, aplicou: false, motivo: "o nome digitado vence a consulta" };
  }
 ...
 * (`hasEnderecoTomador`, em `application/validators/nfsePayload.js`). `xCpl` fica fora: é opcional.
 */
 ...
 * Jesus" no país, e o erro só apareceria como nota emitida no município errado). Está escrito em
 * `lib/municipios/municipioIbge.js` e em `CompanyProvisioningService` — *"a BrasilAPI devolve o
 * município como texto"*. Aqui NÃO se deriva nada: o que se faz é o contrário, aceitar um código
 * que a resposta traga **desde que ele se prove**, contra a lista oficial versionada do IBGE:
 *   1. tem exatamente 7 dígitos;
 *   2. existe na lista (`municipiosIbge.data.js`, extraída da API de Localidades do IBGE);
 ...
 * o resto em silêncio, e o assistente trata o parcial como problema que trava o passo 1. Ou seja,
 * preencher quatro dos cinco campos transformaria uma consulta bem-sucedida em BLOQUEIO da emissão
 * — a consulta virando portão, exatamente o que ela não pode ser. Faltando um pedaço, devolve
 * `endereco: null` e diz o que faltou.
 */
...
export function mensagemEndereco(leitura) {
  ...
  return (
    ...
    + "O servidor só aceita o endereço do tomador COMPLETO — meio endereço seria descartado inteiro, "
    + "então ele fica vazio. Preencha à mão, ou deixe vazio: ele é opcional."
  );
}

/**
 * Aplica o endereço consultado. Mesma regra do nome: o que o contador digitou vence.
 */
 ...
 * tomador BAIXADO/INAPTO é decisão do contador, mas ele precisa saber. Aviso — nunca bloqueio.
 */
