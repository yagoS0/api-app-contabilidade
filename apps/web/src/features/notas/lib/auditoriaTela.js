...
export const FRASE_NOTA_NAO_AVALIADA = Object.freeze({
  ...
  SEM_NUMERO_DE_DPS: "o número da DPS não foi extraído do XML",
  SEM_SERIE_DE_DPS: "a série da DPS não foi extraída do XML",
  NUMERO_DE_DPS_NAO_NUMERICO: "o número da DPS não é numérico",
});
...
export const FRASE_ESPECIE = Object.freeze({
  ...
  NUMERO_REPETIDO: "o mesmo número aparece em mais de uma nota",
  NUMERO_PULADO: "não há nota nesta faixa de números",
  LEITURA_FALHOU: "a leitura do XML não completou",
  // ⚠ Este NÃO é "o XML não traz o campo" — é "o extrator nunca passou por esta linha". A coluna
  // `camposFiscaisExtraidosEm` existe exatamente para desfazer essa ambiguidade, e a frase precisa
  // desfazê-la também: sem isso o contador lê "sem dado" e não procura mais.
  NUNCA_EXTRAIDA: "os campos fiscais nunca foram extraídos desta nota",
});

/** O motivo técnico da extração (vocabulário de `camposFiscaisNfse.MOTIVO`), em português. */
export const FRASE_MOTIVO_EXTRACAO = Object.freeze({
  XML_AUSENTE: "o XML da nota não está guardado",
  XML_ILEGIVEL: "o XML está truncado ou corrompido",
  NAO_E_NFSE: "o XML guardado não é o da NFS-e (provavelmente o de um evento)",
  NENHUM_CAMPO: "o XML tem outro leiaute — nenhum campo foi reconhecido",
  CTRIBNAC_FORA_DA_FORMA: "o código de serviço não tem os 6 dígitos previstos",
  EXTRACAO_LANCOU: "a leitura falhou com erro",
});
...
export function frasesDoAchado(achado, pergunta) {
  ...
  if (dados.especie === "NUMERO_PULADO") {
    const faixa = dados.de === dados.ate ? `nº ${dados.de}` : `nº ${dados.de} a ${dados.ate}`;
    return {
      titulo: `Série ${dados.serie} · ${faixa}`,
      texto: `${base} (${dados.quantidade} número(s), entre a ${dados.antes} e a ${dados.depois}).`,
    };
  }
  if (dados.especie === "NUMERO_REPETIDO") {
    return {
      titulo: `Série ${dados.serie} · nº ${dados.numeroDps}`,
      texto: `${base} (${(dados.notas || []).length} notas).`,
    };
  }
  ...
  if (pergunta?.id === "EMISSAO_FORA_DA_COMPETENCIA") {
    ...
    // ⚠ O desvio ANDA JUNTO porque é ele que separa a virada de mês (−1, o caso normal do serviço
    // prestado em julho e faturado em 1º de agosto) do que merece olhar de verdade.
  ...
  if (pergunta?.id === "NOTA_NAO_LIDA") {
    const porque = dados.motivo ? FRASE_MOTIVO_EXTRACAO[dados.motivo] || dados.motivo : null;
    return {
      titulo: achado?.numero ? `Nota ${achado.numero}` : "Nota",
      texto: porque ? `${base}: ${porque}.` : `${base}.`,
    };
  }
