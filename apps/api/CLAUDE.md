...
Implementação local sobre o snapshot da tarefa Fiscal; ainda exige integração e piloto controlado antes de ativar o executor. Resultado fiscal é CONFIRMADO, NAO_LOCALIZADO, INDETERMINADO, PARCIAL_OU_DIVERGENTE ou NAO_APLICAVEL. `pago/encontrado` aceita null; não converter falha em guia aberta. DAS usa consulta atual com vínculo do documento; circular antiga não confirma pagamento de hoje. PAGTOWEB valida status, PDF e identidade antes de confirmar; ausência de comprovante não prova inadimplência.
...
`entradaComercialPublica` deriva do canal COMERCIAL ativo e das flags existentes no servidor. Menu e coleta desse canal não carregam sessão fiscal, seletor nem rascunho de emissão de um cliente conhecido. Ambiguidade apenas de EMPRESA, com identidade migrada vigente, permite nova solicitação; PESSOA/EM_REVISAO continuam bloqueados. Antes da saída, revalidar canal, vínculo, interlocutor, janela e humano. Nunca aceitar um parâmetro externo como dispensa de RBAC. O principal e as ferramentas fiscais conservam as guardas existentes. `mensagensComerciais` usa o calendário atual do escritório sem anunciar IA após handoff. Testes PostgreSQL de entrada agora geram transcrições opcionais via `WHATSAPP_SIMULATION_REPORT`, apenas fora do Git; ver `../../docs/revisao-comunicacao-cenarios.md`. Sem migration ou reprocessamento de mensagens antigas.

Saudação com caso comercial prévio (17/09): `ColetaComercialWhatsappService` devolve navegação ao menu antes de interpretar campos, inclusive ficha iniciada em outro canal. `navegacaoWhatsapp.js` compartilha saudações/pedidos explícitos; somente frases completas de navegação são interceptadas, preservando “Olá, sou médico e quero abrir empresa”. Interações fora das três origens comerciais seguem seus IDs no menu. Nenhuma saudação altera triagem, ficha, contador de esclarecimentos ou pausa humana. Lead recebe opções novamente, mesmo com menu recente; cliente mantém resposta breve anterior. Teste PostgreSQL reproduz transferência existente aguardando nome, retorno no comercial, replay, retomada e handoff/identificação. Não corrigir apagando dados nem reprocessando entrada já respondida.

Novo pedido em 17/09: segundo número dedicado à entrada de leads. `politicaColetaComercial` aceita todos os remetentes no canal COMERCIAL ativo, com identidade V2/multicanal ON e canalId coincidente; principal conserva a audiência anterior. `resolverCanalEntrada` fornece metadados validados sem segredo para menu/coleta/política de IA. O worker comercial recarrega a relação do canal e bloqueia uso do modelo quando a coleta atende. Não altera classificação, RBAC ou pausas. Em 17/09 às 19h37 UTC o token comercial foi validado (WABA/telefone/aplicativo/webhook); canal e `WHATSAPP_MULTICANAL=1` ativados sobre main `f6fe6a2a`, com PR 68. Entrada real do usuário registrada, sem modelo; pausa humana anterior do interlocutor corretamente impediu resposta automática. Devolver ao automático deve usar `alterarAtendimentoHumano`, com autorização específica, sem apagar histórico nem reprocessar mensagens invalidadas. Repetidas 25 verificações PostgreSQL de jornada/canais/identidade, zero rede/IA. Os estados inativos seguintes são históricos.
