...
async function getAuth() {
  if (!GOOGLE_APPLICATION_CREDENTIALS && !GOOGLE_APPLICATION_CREDENTIALS_JSON) {
    throw new Error("GOOGLE_APPLICATION_CREDENTIALS ou GOOGLE_APPLICATION_CREDENTIALS_JSON ausente");
  }

  if (GOOGLE_APPLICATION_CREDENTIALS_JSON) {
    let credentialsContent;
    try {
      credentialsContent = JSON.parse(String(GOOGLE_APPLICATION_CREDENTIALS_JSON));
    } catch (err) {
      throw new Error(`GOOGLE_APPLICATION_CREDENTIALS_JSON inválido: ${err.message}`);
    }
    if (!credentialsContent?.client_email || !credentialsContent?.private_key) {
      throw new Error("GOOGLE_APPLICATION_CREDENTIALS_JSON sem client_email/private_key");
    }
