...
async function elegivel(tx, user) {
  if (!user || user.status !== 'active' || user.accountType !== 'CLIENT' || ['admin','contador'].includes(user.role)) return false;
  return Boolean(await tx.companyClientUser.findFirst({ where: { userId: user.id, status: 'ACTIVE' }, select: { userId: true } }));
}
...
export async function enviarCodigoAcesso({ to, code }) {
  ...
  await new EmailService().send({ to, subject: 'Seu código de acesso — Altan Contabilidade',
