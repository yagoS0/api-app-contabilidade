...
export async function obterClientesAnalise({portalClientId,de,ate,comparar,client=prisma,agora=new Date()}) {
  ...
  return {ok:true,demonstracao:false,...montarClientes({competenciasFechadas:[...fechados],notas:notas.filter(n=>fechados.has(n.competencia?.toISOString().slice(0,7))).map(n=>({...n,competencia:n.competencia?.toISOString().slice(0,7)})),de,ate,comparar,hoje:agora.toLocaleDateString('en-CA',{timeZone:'America/Sao_Paulo'})})};
}
