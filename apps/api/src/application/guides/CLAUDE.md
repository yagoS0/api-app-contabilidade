...
## Confirmação por WhatsApp — 25/09/2026
AvisoPagamentoService usa exclusivamente os destinatários WhatsApp ativos com opt-in, independentemente do canal padrão das guias. Nunca recorre a e-mail. Confirmação é botão de resposta, sem URL de portal, com token opaco de sete dias vinculado à empresa, guia e contato. ConfirmarPagamentoWhatsappService revalida contato/escopo, registra a declaração CLIENTE e recibo da mensagem em transação serializável, sem inventar data real de pagamento ou gerar baixa. Repetições preservam confirmação já existente. O recálculo conserva seu link separado. Janela fechada permanece pendente, sem envio fora da janela nem template inventado. Testes simulados, nenhum envio real.

## Data declarada e comprovantes — 25/09/2026
Substitui o clique de confirmação imediata descrito acima: botão inicia coleta de data no WhatsApp; somente data válida marca CLIENTE. Data futura é comparada pelo calendário de São Paulo. Comprovante PDF/JPEG/PNG opcional fica vinculado em ArquivoWhatsapp.comprovanteGuiaId, com data declarada e análise separada. Documentos vinculados são preservados além do prazo comum de 90 dias. PDF textual tem tentativa de leitura de data/valor rotulados; fotos e PDFs sem texto seguem conferência humana. Nenhuma leitura transforma declaração em prova oficial, substitui valores da guia ou cria baixa. A conferência aparece em Arquivos recebidos pelo WhatsApp do escritório. SolicitarPagamentosWhatsappService percorre todos os tipos/origens elegíveis nas empresas com rotina pagamento ativa, após a consulta existente, sem novas chamadas SERPRO. Requer migration 20260925190000_comprovante_cliente_whatsapp e geração Prisma. Sem disparos reais nos testes, ainda não publicado. Plano de clareza da emissão em docs/plano-emissao-whatsapp-20260925.md.
