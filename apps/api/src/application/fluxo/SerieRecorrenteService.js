...
/**
 * ⚠⚠ A RECORRÊNCIA DECLARADA — *"essa é a taxa anual que pago de Conselho"*.
 *
 * É o caminho principal para o que o detector NÃO enxerga: a despesa sem nota, e qualquer padrão
 * que não seja mensal. Ela nasce **`PENDENTE`**: uma afirmação não entra no fluxo sozinha, do mesmo
 * jeito que uma observação não entra.
 *
 * ⚠⚠ E ELA NÃO SOBRESCREVE UMA SÉRIE JÁ CONFIRMADA. Quem já foi decidido pelo contador continua
 * como está; a declaração vira linha de conferência ao lado. *"O observado vence"* é decisão do
 * dono — e uma declaração que rebaixasse uma série ATIVA para PENDENTE apagaria a decisão dele.
 *
 ...
 * os estrutura hoje é a pessoa preenchendo a tela.
 */
