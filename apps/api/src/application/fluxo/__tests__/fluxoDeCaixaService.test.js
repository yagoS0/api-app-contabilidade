...
it("saldo inicial inclui movimentos anteriores à janela visual, sem corte de 12 meses", async () => {
  ...
  expect(r.meses[0].competencia).toBe("2026-04");
  expect(r.meses[0].saldo.inicial).toBe(960);
...
it("duas projeções de meio centavo têm linhas, totais e saldo coerentes em 0,02", async () => {
  const client = clientDe({ series: [
    serie({ id: "s-centavo1", lado: LADO.RECEITA, baseDaObservacao: { n: 3, mediana: 0.005 } }),
    serie({ id: "s-centavo2", lado: LADO.RECEITA, baseDaObservacao: { n: 3, mediana: 0.005 } }),
  ] });
  ...
  const r = await montar(client);
  const mes = doMes(r, "2026-08");
  expect(mes.linhas.filter(l => l.direcao === DIRECAO.ENTRADA).map(l => l.valor)).toEqual([0.01,0.01]);
  expect(mes.totais.previsao.entrada).toBe(0.02);
