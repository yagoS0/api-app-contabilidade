...
if (posRetomar >= 0) {
  ...
  try {
    ...
    const botaoGuias = menuDaEmpresa.linhas.find(o => o.titulo === 'Guias em aberto');
    assert.match(botaoGuias.id, /^altan\.ctx\.v1:/);
    const antesDasGuias = await checkpoint(jornada.atendimento.id);
    const pedidoGuias = await jornada.receberWebhook('Guias em aberto', { interacao: botaoGuias });
    ...
    await jornada.receberWebhook('Guias em aberto', { interacao: botaoGuias, providerMessageId: pedidoGuias.mensagem.providerMessageId });
    ...
    const guiasDaOutra = jornada.envios.at(-1).linhas.find(o => o.titulo === 'Guias em aberto');
