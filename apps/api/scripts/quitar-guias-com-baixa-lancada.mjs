...
//  3. **Nunca rebaixa**: guia paga por SERPRO (prova) ou MANUAL (o contador) não é tocada.
//     ⚠⚠ A paga pelo CLIENTE É corrigida — o clique não é prova, o lançamento do contador é. Foi o
//     caso do *"pagamento de 1.876,46 que não faz sentido"*: duas guias confirmadas no mesmo dia,
//     ambas com a data do clique, uma delas provada no razão em 13/06/2026.
...
import { baixaPodeDatarAGuia, guiaQuitadaPelaBaixa } from "../src/application/guides/lib/guiaQuitadaPelaBaixa.js";
...
for (const e of provisoes) {
  ...
  const guia = await p.guide.findFirst({
    ...
    select: { id: true, tipo: true, competencia: true, valor: true, paymentStatus: true,
              paymentStatusSource: true, paymentConfirmedAt: true, liberadaCliente: true },
  });
  if (!guia) { semAlvo += 1; continue; }
  if (!baixaPodeDatarAGuia(guia)) { jaPagas += 1; continue; }
  aQuitar.push({ e, guia, quando: e.baixas[0].data });
}

console.log("empresa".padEnd(27), "tipo".padEnd(8), "comp".padEnd(9), "valor".padStart(13), "de".padEnd(11), "data", "         cliente vê");
for (const { e, guia, quando } of aQuitar) {
  console.log(
    ...
    `${guia.paymentStatus}${guia.paymentStatusSource === "CLIENTE" ? "/cli" : ""}`.padEnd(11),
    `${dia(guia.paymentConfirmedAt)} → ${dia(quando)}`, "  ", guia.liberadaCliente ? "SIM" : "não"
  );
  if (APLICAR) {
