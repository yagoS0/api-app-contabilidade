...
Os 24 cenários de banco cobrem concorrência, importação privada, histórico, conflitos de versão, preços, aceite, PDF, documento/assinatura, avulso sem carteira, troca de CNPJ, envio sem repetição, função seguida de resposta, intervenção humana, vínculo manual e conferências. Há bloqueio explícito de rede no teste de banco. A regressão integrada local passou 823 testes em 42 suítes de API; os componentes comerciais passaram 29 testes e o importador passou 4 testes de CLI. O build de produção da interface também passou.
